...
class Version_Loader
{
    ...
    {
        $data = $this->select_newest_file(isset($this->classmap[$class_name]) ? $this->classmap[$class_name] : null, $this->find_psr4_file($class_name));
        if (!isset($data)) {
            return null;
        }
        return $data['path'];
    ...
    public function load_filemap()
    {
        ...
        foreach ($this->filemap as $file_identifier => $file_data) {
            if (empty($GLOBALS['__composer_autoload_files'][$file_identifier])) {
                ...
                $GLOBALS['__composer_autoload_files'][$file_identifier] = \true;
            }
        }
    }
    /**
    ...
    private function find_psr4_file($class_name)
    {
        ...
        for ($class_namespace = \substr($class_name, 0, $class_index); !empty($class_namespace); $class_namespace = \substr($class_namespace, 0, \strrpos($class_namespace, '\\'))) {
            ...
            foreach ($data['path'] as $path) {
                ...
                if (\file_exists($path)) {
                    return array('version' => $data['version'], 'path' => $path);
                }
            }
        }
        return null;
 ...
 * @internal
 */
\class_alias('WC_USPS\\Version_Loader', 'Version_Loader', \false);
