...
use WC_USPS\Composer\Autoload\ClassMapGenerator;
use WC_USPS\Composer\Composer;
use WC_USPS\Composer\Config;
use WC_USPS\Composer\Installer\InstallationManager;
use WC_USPS\Composer\IO\IOInterface;
use WC_USPS\Composer\Package\PackageInterface;
use WC_USPS\Composer\Repository\InstalledRepositoryInterface;
use WC_USPS\Composer\Util\Filesystem;
use WC_USPS\Composer\Util\PackageSorter;
/**
...
class AutoloadGenerator
{
     ...
     */
    public function dump(Composer $composer, Config $config, InstalledRepositoryInterface $localRepo, PackageInterface $mainPackage, InstallationManager $installationManager, $targetDir, $scanPsrPackages = \false, $suffix = null)
    {
    ...
    protected function getPathCode(Filesystem $filesystem, $basePath, $vendorPath, $path)
    {
        ...
        return $baseDir . (\false !== $path ? \var_export($path, \true) : '');
    }
    /**
    ...
    protected function parseAutoloadsType(array $packageMap, $type, PackageInterface $mainPackage)
    {
        ...
        foreach ($packageMap as $item) {
            ...
            if (\in_array($type, array('psr-4', 'psr-0'), \true) && isset($autoload[$type]) && \is_array($autoload[$type])) {
                foreach ($autoload[$type] as $namespace => $paths) {
                    $paths = \is_array($paths) ? $paths : array($paths);
                    foreach ($paths as $path) {
                        $relativePath = empty($installPath) ? empty($path) ? '.' : $path : $installPath . '/' . $path;
                        $autoloads[$namespace][] = array('path' => $relativePath, 'version' => $package->getVersion());
                    }
                }
            }
            if ('classmap' === $type && isset($autoload['classmap']) && \is_array($autoload['classmap'])) {
                foreach ($autoload['classmap'] as $paths) {
                    ...
                    foreach ($paths as $path) {
                        $relativePath = empty($installPath) ? empty($path) ? '.' : $path : $installPath . '/' . $path;
                        $autoloads[] = array('path' => $relativePath, 'version' => $package->getVersion());
                    }
                }
            }
            if ('files' === $type && isset($autoload['files']) && \is_array($autoload['files'])) {
                foreach ($autoload['files'] as $paths) {
                    ...
                    foreach ($paths as $path) {
                        $relativePath = empty($installPath) ? empty($path) ? '.' : $path : $installPath . '/' . $path;
                        $autoloads[$this->getFileIdentifier($package, $path)] = array('path' => $relativePath, 'version' => $package->getVersion());
                    }
                }
            }
        }
    ...
    {
        $processor = new \Automattic\Jetpack\Autoloader\AutoloadProcessor(function ($path, $excludedClasses, $namespace) use($basePath) {
            $dir = $this->filesystem->normalizePath($this->filesystem->isAbsolutePath($path) ? $path : $basePath . '/' . $path);
            return ClassMapGenerator::createMap(
                $dir,
                $excludedClasses,
                null,
                // Don't pass the IOInterface since the normal autoload generation will have reported already.
        ...
        return array('psr-4' => $processor->processPsr4Packages($autoloads, $scanPsrPackages), 'classmap' => $processor->processClassmap($autoloads, $scanPsrPackages), 'files' => $processor->processFiles($autoloads));
    }
    /**
    ...
    {
        $files = array('autoload_functions.php', 'class-autoloader-handler.php', 'class-classes-handler.php', 'class-files-handler.php', 'class-plugins-handler.php', 'class-version-selector.php');
    ...
    private function writeAutoloaderFiles($outDir, $suffix)
    {
        ...
        \Automattic\Jetpack\Autoloader\AutoloadFileWriter::copyAutoloaderFiles($this->io, $outDir, $suffix);
    }
    /**
    ...
    private function writeManifests($outDir, $processedAutoloads)
    {
        ...
        $manifestFiles = array('classmap' => 'jetpack_autoload_classmap.php', 'psr-4' => 'jetpack_autoload_psr4.php', 'files' => 'jetpack_autoload_filemap.php');
        foreach ($manifestFiles as $key => $file) {
            ...
            $content = \Automattic\Jetpack\Autoloader\ManifestGenerator::buildManifest($key, $file, $processedAutoloads[$key]);
            if (empty($content)) {
                continue;
            }
