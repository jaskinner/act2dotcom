...
class AutoloadProcessor
{
    ...
    public function processClassmap($autoloads, $scanPsrPackages)
    {
            ...
            foreach ($autoloads['psr-4'] as $namespace => $sources) {
                ...
                foreach ($sources as $source) {
                    ...
                    foreach ($classmap as $class => $path) {
                        $processed[$class] = array('version' => $source['version'], 'path' => \call_user_func($this->pathCodeTransformer, $path));
                    }
                }
            }
        }
        ...
        if (!empty($autoloads['classmap'])) {
            foreach ($autoloads['classmap'] as $package) {
                ...
                foreach ($classmap as $class => $path) {
                    $processed[$class] = array('version' => $package['version'], 'path' => \call_user_func($this->pathCodeTransformer, $path));
                }
            }
        }
        return $processed;
    ...
    public function processPsr4Packages($autoloads, $scanPsrPackages)
    {
        ...
        foreach ($autoloads['psr-4'] as $namespace => $packages) {
            ...
            $processed[$namespace] = array('version' => $package['version'], 'path' => $paths);
        }
        return $processed;
    }
    /**
    ...
    public function processFiles($autoloads)
    {
        ...
        foreach ($autoloads['files'] as $file_id => $package) {
            $processed[$file_id] = array('version' => $package['version'], 'path' => \call_user_func($this->pathCodeTransformer, $package['path']));
        }
        return $processed;
    }
}
