...
use function count;
use function max;
use WC_USPS\Psr\Log\LoggerAwareInterface;
use WC_USPS\Psr\Log\LoggerInterface;
use WC_USPS\Psr\Log\NullLogger;
use function reset;
use function usort;
/**
 * Actual packer.
    ...
    private function packRotation(int $boxWidth, int $boxLength) : PackedBox
    {
        ...
        while ($items->count() > 0) {
            ...
            if ($preliminaryLayer->getDepth() === $preliminaryLayer->getItems()[0]->getDepth()) {
                ...
                $layers[] = $this->layerPacker->packLayer($items, $packedItemList, 0, 0, $layerStartDepth, $boxWidth, $boxLength, $this->box->getInnerDepth() - $layerStartDepth, $preliminaryLayer->getDepth(), \true);
            }
        }
